...
async def async_setup_entry(
    ...
    for manager in entry.runtime_data.panels.values():
        entities: list[BrilliantPanelEntity] = [WakeWordSelect(manager)]
        if manager.fleet.ha_control_enabled:
            entities.append(SceneSelect(manager))
        else:
            async_remove_gated_entity(hass, "select", f"{manager.store.management_id}_scene")
