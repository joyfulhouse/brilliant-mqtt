...
import numpy as np
from . import alsa_mic as sc  # ALSA/arecord shim (panel has no PulseAudio)
...
async def main() -> None:
    ...
    parser.add_argument(
        "--stream-input-device",
        help="Optional separate device for the audio STREAMED to HA for STT. "
        "When set, --audio-input-device is used only for wake detection and this "
        "device feeds STT. Lets wake run on the raw chain while STT streams "
        "echo-cancelled audio (wake happens in quiet; echo only matters streaming).",
    )
    parser.add_argument(
        "--list-input-devices",
        action="store_true",
    ...
    # Optional separate source for the audio streamed to HA (STT).
    stream_mic = sc.get_microphone(args.stream_input_device) if args.stream_input_device else None
    ...
    process_audio_thread = threading.Thread(
        ...
        args=(state, mic, args.audio_input_block_size, stream_mic),
        daemon=True,
    )
...
def process_audio(state: ServerState, mic, block_size: int, stream_mic=None):
    ...
    try:
        ...
        with ExitStack() as audio_stack:
            mic_in = audio_stack.enter_context(
                mic.recorder(samplerate=16000, channels=n_channels, blocksize=block_size)
            )
            # Optional separate STREAM source (read in lockstep with the wake
            # mic). Wake detection runs on `mic` (raw chain → reliable wake);
            # STT streams `stream_in` (echo-cancelled). Both are 16 kHz realtime.
            stream_in = None
            if stream_mic is not None:
                _LOGGER.debug("Opening separate STREAM input device: %s", stream_mic.name)
                stream_in = audio_stack.enter_context(
                    stream_mic.recorder(samplerate=16000, channels=1, blocksize=block_size)
                )
            while True:
                ...
                # Audio streamed to HA for STT: the separate stream source if
                # configured (echo-cancelled), else the same primary channel.
                if stream_in is not None:
                    sraw = stream_in.record(block_size).reshape(-1)
                    stream_chunk = (np.clip(sraw * mic_vol_scalar, -1.0, 1.0) * 32767.0).astype("<i2").tobytes()
                else:
                ...
                if (not wake_words) or (state.wake_words_changed and state.wake_words):
                    ...
                    # Capture a local ref: state.satellite is shared with the asyncio
                    # thread, which sets it to None on HA disconnect. Reading it once
                    # avoids a cross-thread TOCTOU (check non-None, then it becomes None
                    # before the .state access). This whole block is cosmetic WebUI
                    # entity-state sync, so any failure here must never kill wake
                    # detection — wrap it and continue.
                    ...
                    if satellite is not None:
                        try:
                            ...
                            # Wake Word 1
                            if satellite.state.sensitivity_1_number_entity is not None:
                                _LOGGER.debug("  → Syncing Wake Word 1 entity to value %.3f", state.wake_word_1_threshold)
                                satellite.state.sensitivity_1_number_entity.sync_with_state()
                                _LOGGER.debug("  ✅ Wake Word 1 entity now has value %.3f", satellite.state.sensitivity_1_number_entity.value)

                            # Wake Word 2
                            if satellite.state.sensitivity_2_number_entity is not None:
                                _LOGGER.debug("  → Syncing Wake Word 2 entity to value %.3f", state.wake_word_2_threshold)
                                satellite.state.sensitivity_2_number_entity.sync_with_state()
                                _LOGGER.debug("  ✅ Wake Word 2 entity now has value %.3f", satellite.state.sensitivity_2_number_entity.value)

                            # Stop Word
                            if satellite.state.stop_sensitivity_number_entity is not None:
                                _LOGGER.debug("  → Syncing Stop Word entity to value %.3f", state.stop_word_threshold)
                                satellite.state.stop_sensitivity_number_entity.sync_with_state()
                                _LOGGER.debug("  ✅ Stop Word entity now has value %.3f", satellite.state.stop_sensitivity_number_entity.value)
                            ...
                            for entity in [
                                satellite.state.sensitivity_1_number_entity,
                                satellite.state.sensitivity_2_number_entity,
                                satellite.state.stop_sensitivity_number_entity,
                            ]:
                                if entity is not None:
                                    satellite.send_messages([NumberStateResponse(key=entity.key, state=entity.value)])  # type: ignore[attr-defined]
                                    _LOGGER.debug("  → Pushed value %.3f for entity %d", entity.value, entity.key)
                        except Exception as e:
                            _LOGGER.debug("Skipped WebUI entity sync (HA disconnected mid-sync): %s", e)
                ...
                try:
                    ...
                    # Stream the (optionally echo-cancelled) stream_chunk to HA;
                    # wake detection below still uses audio_chunk (raw chain).
                    state.satellite.handle_audio(stream_chunk, audio_chunk_2)
