...
class VoiceSatelliteProtocol(APIServer):
    ...
    def wakeup(self, wake_word: Union[MicroWakeWord, OpenWakeWord]) -> None:
        ...
        self._pipeline_active = True
        self.duck()
        if os.environ.get("LVA_INSTANT_WAKE") == "1":
            # Snappy wake: open the mic immediately and play the chime in
            # parallel instead of waiting for it to finish. Safe only with the
            # AEC daemon active — it cancels the chime echo from the mic so the
            # chime doesn't bleed into STT. Gives a "just start talking" feel.
