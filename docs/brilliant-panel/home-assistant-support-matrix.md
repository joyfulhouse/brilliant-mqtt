...
| Native HA light tile on a physical slider | **Source-eligible; live blocked** | Three stale physical-Control-owned HA `LIGHT` records were offered as assignable targets in the Office slider UI, but were not bound or operated. The four-process VC candidate, type-19 link, non-root supervisor shape, and credential handoff are implemented/documented off-panel; no official VC/light, account/service, real-ARM bootstrap, online routing, binding, persistence, or cleanup proof exists. See [slider feasibility](slider-bridge-feasibility.md). |
