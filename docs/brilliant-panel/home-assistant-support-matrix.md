...
| Native HA light tile on a physical slider | **Source-eligible; live blocked** | Three stale physical-Control-owned HA `LIGHT` records were offered as assignable targets in the Office slider UI, but were not bound or operated. The four-process VC candidate, type-19 link, credential handoff, exact captured-firmware pre-start, and bounded non-root bootstrap reference are implemented/documented. The separate coordinated light-session unit/approval is not; no official VC/light, account/install, real-ARM bootstrap, online routing, binding, persistence, or cleanup proof exists. See [slider feasibility](slider-bridge-feasibility.md). |
...
was no longer running. The operator confirmed that the three Backyard records
were visible on the ordinary Backyard room screen and, in a later explicit
check, were offered as assignable targets in the Office slider UI. None was
bound or operated. This live-confirms native light rendering, room placement,
and selector admission for the deprecated ownership model. It does not confirm
a Virtual Control, online command routing, saved binding, physical operation,
or HA feedback.
