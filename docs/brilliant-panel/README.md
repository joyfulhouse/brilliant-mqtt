...
5. **A Virtual Control is a supervised vassal set, not a direct Python runner.** Stock-path ARM execution proved that a dedicated non-root Emperor can start only the message bus first; its captured process manager then creates enabled discovery/bootstrap vassals in mode-`0700` writable directories. The E2E candidate also needs the grouped configuration host for its type-19 Device Configuration link. The root-to-runtime credential handoff is now implemented off-panel; account/service creation, real-ARM bootstrap/home assignment, remote-bridge isolation, and locality remain live gates.
