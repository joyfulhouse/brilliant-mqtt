...
4. **Physical-Control HA hosting is rejected.** Although bundled adapters host typed peripherals, adding a manager to a real Control co-managed physical hardware, added bus load, threatened load responsiveness, and did not reliably admit or propagate tiles. The supported baseline remains the HA-owned MQTT scene/mode bridge.
5. **Shipped partner stubs form a separate native-type research path.** The firmware's ordinary partner configuration can declare `PeripheralInfo(stubbed=True)` and let the naturally owned stock host materialize native types. Read-only Office inventory identifies an unused Hunter Douglas `SHADE` ring and dormant RemoteLock `LOCK` ring as bounded candidates. This does not authorize production mutation: owner/relay, authoritative HA feedback, availability, cleanup, and disposable-home gates remain open. See [native partner-stub feasibility](native-partner-stub-feasibility.md).
6. **A Virtual Control is still required for generic arbitrary native hosting.** It is a supervised vassal set, not a direct Python runner. Stock-path ARM execution proved that a dedicated non-root Emperor can start only the message bus first; its captured process manager then creates enabled discovery/bootstrap vassals. The E2E candidate also needs the grouped configuration host for its type-19 Device Configuration link. Credential handoff, exact no-start stock preparation, staged app/vendor validation, the bounded bootstrap unit, and the separate coordinated one-light unit/coordinator are implemented. Their captured-ARM preparation/options and off-panel service shape pass; the exact unit has not been staged on Office. Official identity, account/install review, live bootstrap/home assignment, remote-bridge isolation, online HA routing, cleanup, and locality remain live gates.
7. **Physical slider and gesture bindings are configuration objects, not ordinary state variables.** They can target lights, groups, scenes, modes, and native `SHADE` peripherals. The lack of a press-event variable explains why simple bus observation cannot expose every gesture as an HA event.
8. **Camera/intercom is a media subsystem, not a boolean camera entity.** It combines raw camera hardware, GStreamer, WebRTC/SDP session state, RTSP, remote-media peripherals, and privacy gates. A stock camera stub may help admit a tile, but it does not answer arbitrary HA live-view sessions; media ownership remains separate from the shade/lock path.
9. **Cloud removal is capability-specific.** Wired loads, mesh loads, panel controls, MQTT, and the scene/mode bridge use local paths. Partner account linking, some media relay paths, weather/art catalogs, Alexa, and OTA discovery remain cloud-backed unless separately replaced. Virtual Control locality is unproven and must not be inferred from the local scene bridge. A PowerView-compatible local service is a cover-specific fallback if the generic stub data plane cannot be made authoritative.
