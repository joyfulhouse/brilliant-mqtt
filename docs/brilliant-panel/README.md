...
5. **A Virtual Control is a supervised vassal set, not a direct Python runner.** Stock-path ARM execution proved that Emperor starts only the message bus first; its captured process manager then creates enabled discovery/bootstrap vassals. The E2E candidate also needs the grouped configuration host for its type-19 Device Configuration link. Runtime-user credential handoff, real-ARM bootstrap/home assignment, remote-bridge isolation, and locality remain live gates.
