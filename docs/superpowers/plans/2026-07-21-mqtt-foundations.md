...
Before `write_env` or `ensure_configs` writes any file, inspect only `MQTT_TLS_ENABLED` in both the live and OTA-staged existing environments. Recognize the agent's strict boolean spellings through valid single or double systemd quotes; conservatively refuse escaped, continued, or otherwise ambiguous assignments. If either environment enables TLS or is ambiguous while the desired environment does not enable TLS, fail closed with `mqtt_tls_downgrade_refused` before mkdir/upload/daemon-reload. Any install, update, or code-less repair path that can deploy the payload before `ensure_configs` must run the same guard before `deploy_payload`; retain the guard inside `write_env` and `ensure_configs` as the final write boundary. A TLS-enabled desired environment needs no probe. Full TLS adoption and UI round-tripping remain Plan 2 scope.
