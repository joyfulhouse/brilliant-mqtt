...
DEB_OPENBLAS="http://deb.debian.org/debian/pool/main/o/openblas/libopenblas0-pthread_0.3.13+ds-3+deb11u1_armhf.deb"
DEB_GFORTRAN="http://deb.debian.org/debian/pool/main/g/gcc-10/libgfortran5_10.2.1-6_armhf.deb"
DEB_LIBSTDCXX="http://ports.ubuntu.com/ubuntu-ports/pool/main/g/gcc-12/libstdc++6_12.3.0-1ubuntu1~22.04.3_armhf.deb"
...
extract_deb() {
  ...
  find "$deb_dir" -name '*.so*' ! -type d | while read -r so; do
    cp -n "$so" "$DEST/libs/"
  done
}

extract_deb "${DEB_OPENBLAS}"
extract_deb "${DEB_GFORTRAN}"
